import { jsonResponse, fbGraph, d1All, d1First, d1Run, newId, logError } from './utils.js';

export async function handleAuthLogin(request, env) {
  try {
    const redirectUri = `${env.SITE_URL_BACKEND}/auth/callback`;
    const scope = 'pages_show_list,pages_manage_posts,pages_read_engagement,pages_manage_metadata,pages_messaging,business_management';
    const authUrl = `https://www.facebook.com/${env.FB_GRAPH_VERSION}/dialog/oauth?client_id=${env.FB_APP_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&config_id=${env.FB_CONFIG_ID}&scope=${encodeURIComponent(scope)}&response_type=code`;
    return Response.redirect(authUrl, 302);
  } catch (err) {
    await logError(env, 'auth.handleAuthLogin', err, {});
    return jsonResponse({ error: 'Login redirect failed: ' + err.message }, 500);
  }
}

export async function handleAuthCallback(request, env) {
  const url = new URL(request.url);
  const code = url.searchParams.get('code');
  const path = url.pathname;

  if (!code) {
    await logError(env, 'auth.handleAuthCallback', new Error('Missing code param'), { path });
    return jsonResponse({ error: 'Missing code' }, 400);
  }

  try {
    const redirectUri = `${env.SITE_URL_BACKEND}/auth/callback`;

    const tokenRes = await fbGraph(env, 'oauth/access_token', {
      client_id: env.FB_APP_ID,
      client_secret: env.FB_APP_SECRET,
      redirect_uri: redirectUri,
      code
    });
    if (tokenRes.error) {
      await logError(env, 'auth.handleAuthCallback:oauth/access_token', new Error(tokenRes.error.message), { path, fbError: tokenRes.error });
      return jsonResponse({ error: tokenRes.error.message }, 400);
    }

    const longLived = await fbGraph(env, 'oauth/access_token', {
      grant_type: 'fb_exchange_token',
      client_id: env.FB_APP_ID,
      client_secret: env.FB_APP_SECRET,
      fb_exchange_token: tokenRes.access_token
    });
    if (longLived.error) {
      await logError(env, 'auth.handleAuthCallback:fb_exchange_token', new Error(longLived.error.message), { path, fbError: longLived.error });
      return jsonResponse({ error: longLived.error.message }, 400);
    }

    const profile = await fbGraph(env, 'me', { fields: 'id,name,email', access_token: longLived.access_token });
    if (profile.error) {
      await logError(env, 'auth.handleAuthCallback:me', new Error(profile.error.message), { path, fbError: profile.error });
      return jsonResponse({ error: profile.error.message }, 400);
    }

    let user = await d1First(env, `SELECT * FROM users WHERE fb_user_id = ?`, [profile.id]);
    if (!user) {
      const id = newId();
      await d1Run(env, `INSERT INTO users (id, fb_user_id, name, email) VALUES (?, ?, ?, ?)`,
        [id, profile.id, profile.name ?? null, profile.email ?? null]);
      user = { id, fb_user_id: profile.id, name: profile.name ?? null, email: profile.email ?? null };
    }

    const pagesRes = await fbGraph(env, 'me/accounts', { access_token: longLived.access_token, fields: 'id,name,access_token,perms' });
    const grantsRes = await fbGraph(env, 'me/permissions', { access_token: longLived.access_token });

    if (pagesRes.error) {
      await logError(env, 'auth.handleAuthCallback:me/accounts', new Error(pagesRes.error.message), { path, userId: user.id, fbError: pagesRes.error });
    } else if (!pagesRes.data || pagesRes.data.length === 0) {
      await logError(env, 'auth.handleAuthCallback:me/accounts', new Error('me/accounts returned empty'), {
        path, userId: user.id,
        permissions: grantsRes?.data || null,
        hint: 'Check FB_CONFIG_ID business-portfolio page claim/admin access, or scope grant.'
      });
    }

    if (pagesRes.data) {
      for (const p of pagesRes.data) {
        try {
          const existing = await d1First(env, `SELECT id FROM pages WHERE page_id = ?`, [p.id]);
          if (existing) {
            await d1Run(env, `UPDATE pages SET user_id = ?, page_name = ?, access_token = ? WHERE page_id = ?`,
              [user.id, p.name ?? null, p.access_token ?? null, p.id]);
          } else {
            await d1Run(env, `INSERT INTO pages (id, user_id, page_id, page_name, access_token) VALUES (?, ?, ?, ?, ?)`,
              [newId(), user.id, p.id, p.name ?? null, p.access_token ?? null]);
          }
          const subRes = await fbGraph(env, `${p.id}/subscribed_apps`, {
            subscribed_fields: 'feed,messages',
            access_token: p.access_token
          }, 'POST');
          if (subRes.error) {
            await logError(env, 'auth.handleAuthCallback:subscribed_apps', new Error(subRes.error.message), {
              path, userId: user.id, pageId: p.id, fbError: subRes.error
            });
          }
        } catch (pageErr) {
          await logError(env, 'auth.handleAuthCallback:page-loop', pageErr, { path, userId: user.id, pageId: p.id });
        }
      }
    }

    const sessionToken = crypto.randomUUID();
    await env.SESSIONS.put(`session:${sessionToken}`, user.id, { expirationTtl: 60 * 60 * 24 * 30 });

    return Response.redirect(`${env.SITE_URL}/dashboard?token=${sessionToken}`, 302);
  } catch (err) {
    await logError(env, 'auth.handleAuthCallback (uncaught)', err, { path });
    return jsonResponse({ error: 'Login failed: ' + err.message }, 500);
  }
}
